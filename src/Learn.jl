function learn_optim(index::Int, model::BSTModel, training_df::DataFrame; 
    ...
    model.α = p_best[1:9]
    G = model.G
    idx = findfirst(x->x=="FVIIa",model.total_species_list)   # 10
    G[idx, 4] = p_best[10]
    idx = findfirst(x->x=="AT",model.total_species_list)      # 11
    G[idx, 9] = p_best[11]
    idx = findfirst(x->x=="TFPI",model.total_species_list)    # 12
    G[idx, 1] = -1*p_best[12]
